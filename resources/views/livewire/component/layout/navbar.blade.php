                    ...
                    @if (!isset($menu['menus']))
                        <li class="nav-item">
                            <a class="nav-link" href="{{ $menu['url'] }}" key="t-{{ Str::slug($menu['name']) }}">
                                <i class="{{ $menu['icon'] }} me-2"></i><span>{!! $menu['name'] !!}</span>
                            </a>
                        </li>
                    @else
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle arrow-none" href="#"
                                ...
                                <i class="{{ $menu['icon'] }} me-2"></i><span key="t-{{ Str::slug($menu['name']) }}">{{ $menu['name'] }}</span><div class="arrow-down"></div>
                            </a>
                                            ...
                                            <div class="dropdown-menu">
                                                @foreach ($subMenu['menus'] as $subSubMenu)
                                                    <a class="dropdown-item" href="{{ $subSubMenu['url'] }}">
                                                        <i class="{{ $subSubMenu['icon'] }} me-2"></i>{!! $subSubMenu['name'] !!}
                                                    </a>
                                                @endforeach
                                            </div>
                                        </div>
